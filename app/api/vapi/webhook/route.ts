  ...
  try {
    ...
    if (payload.message?.assistantId || (payload as any).assistantId) {
      ...
      const { data: byAssistant, error: assistantError } = await supabase
        .from('businesses')
        .select('id, name, vapi_assistant_id, niche_type')
    ...
    switch (eventType) {
      ...
      case 'function-call':
        return await handleFunctionCall(payload, businessId, business?.name);
...
async function handleFunctionCall(payload: VAPIWebhookPayload, businessId: string, businessName?: string): Promise<NextResponse> {
  ...
  switch (fn.name) {
    ...
    case 'book_appointment':
      return await handleBookAppointment(fn.parameters, businessId, payload.call?.id, businessName);
    ...
    case 'answer_question':
      return await handleGetFaq(fn.parameters?.query || fn.parameters?.question || '', businessId);
...
async function handleBookAppointment(params: any, businessId: string, callId?: string, businessName?: string): Promise<NextResponse> {
  const {
    customer_name,
    customer_phone,
    treatment_type,
  ...
  // Fire-and-forget: trigger the calendar sync
  ...
  // Fire-and-forget: send confirmation SMS immediately
  if (customer_phone) {
    const confirmationMsg = `Hi ${customer_name?.split(' ')[0] || 'there'}! Your ${treatment_type || 'appointment'} at ${businessName || 'us'} is confirmed for ${formattedTime}. Reply STOP to opt out.`;
    fetch(`${appUrl}/api/messages/send`, {
      method: 'POST',
    ...
    // Schedule 24h reminder follow-up
    ...
    if (reminderTime > new Date()) {
      void supabase.from('follow_ups').insert({
        ...
        message_template: `Hi ${customer_name?.split(' ')[0] || 'there'}! Reminder: your ${treatment_type || 'appointment'} is tomorrow at ${appointmentTime.toLocaleString('en-US', { hour: 'numeric', minute: '2-digit', weekday: 'short' })}. Reply STOP to opt out.`,
      });
    }
  }

  // End the call after booking — fire-and-forget via Vapi API
  if (callId) {
  ...
  return NextResponse.json({
    results: [{
      ...
      result: {
        success: true,
        appointment_id: appointment.id,
        message: `Perfect! Your ${treatment_type || 'appointment'} is confirmed for ${formattedTime}. You'll receive a confirmation text shortly. Have a great day, goodbye!`
      }
    }]
  });
}

async function searchKnowledgeBase(businessId: string, query: string, category?: string): Promise<any[]> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/knowledge/search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ business_id: businessId, query, category, limit: 5 }),
    });
    if (res.ok) {
...
async function handleGetServices(businessId: string): Promise<NextResponse> {
  // Try RAG knowledge base first (service + pricing categories)
  const ragResults = await searchKnowledgeBase(businessId, 'services pricing treatments offerings', 'service');
  const pricingResults = await searchKnowledgeBase(businessId, 'pricing rates cost', 'pricing');
  const allRag = [...ragResults, ...pricingResults];

  if (allRag.length > 0) {
    const knowledgeText = allRag.map(r => `${r.title}: ${r.content}`).join('\n\n');
    return NextResponse.json({
      results: [{
  ...
  // Fallback to treatment_templates table
...
async function handleGetFaq(query: string, businessId: string): Promise<NextResponse> {
  // Search knowledge base across all categories for the query
  const results = await searchKnowledgeBase(businessId, query || 'frequently asked questions');

  if (results.length > 0) {
    const answer = results.map(r => `${r.title}: ${r.content}`).join('\n\n');
    return NextResponse.json({
      results: [{
        toolCallId: 'getFaq',
        result: { answer, message: answer }
      }]
    });
  }
  ...
  return NextResponse.json({
    results: [{
      toolCallId: 'getFaq',
      result: {
        answer: "I don't have specific information on that right now. Let me connect you with our team who can answer that directly.",
        message: "I don't have specific information on that right now. Let me connect you with our team who can answer that directly."
      }
    }]
  });
}
