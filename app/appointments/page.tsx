...
import { supabase, type Appointment as AppointmentType, type FollowUp, type Message } from '@/lib/supabase';
...
export default function AppointmentsPage() {
    ...
    async function loadAppointments() {
        try {
            ...
            // Apply business filter if available
            if (businessId) {
            ...
            // Apply source filter - handle both 'ai-agent' and 'ai_phone'
            if (filters.source !== 'all') {
                if (filters.source === 'ai-agent') {
            ...
            const mappedAppointments: AppointmentWithDetails[] = (data || []).map((appt: any) => {
                ...
                return {
                    ...
                    follow_up_status: 'pending' as const, // Will be populated by follow-ups query
                    follow_up_messages: [],
                    client_reply: null,
                };
            });

            // Load follow-ups and messages for each appointment
            await Promise.all(mappedAppointments.map(async (appt) => {
                // Load follow-ups
                ...
                if (followUps && followUps.length > 0) {
                    const completedCount = followUps.filter(f => f.status === 'completed' || f.status === 'sent').length;
                    if (completedCount === followUps.length) {
                        appt.follow_up_status = 'completed';
                    } else if (completedCount > 0) {
                        appt.follow_up_status = 'in_progress';
                    }

                    appt.follow_up_messages = followUps.map(f => ({
                        id: f.id,
                ...
                // Load client reply (most recent inbound message)
                if (appt.client_id) {
    ...
    const getStatusBadge = (status: AppointmentType['status']) => {
        ...
        return (
            <span className={`px-2.5 py-1.5 rounded-md text-[11px] font-semibold uppercase tracking-wide border border-transparent ${styles[status] || 'bg-[var(--foreground-muted)]/10 text-[var(--foreground-muted)]'}`}>
