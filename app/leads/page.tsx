...
export default function LeadsPage() {
    ...
    async function loadLeads() {
        try {
            ...
            const mappedLeads: LeadWithUI[] = (data.leads || []).map((lead: any) => ({
                ...lead,
    ...
    async function convertToClient(lead: LeadWithUI) {
        ...
        try {
            const res = await fetch('/api/leads', {
                ...
                body: JSON.stringify({ lead, business_id: businessId }),
            });
    ...
    async function markAsLost(leadId: string, reason?: string) {
        ...
        try {
            const res = await fetch('/api/leads', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: leadId, status: 'lost', lost_reason: reason || 'Manually marked as lost' }),
            });
    ...
    async function updateLeadStatus(leadId: string, newStatus: string) {
        try {
            const res = await fetch('/api/leads', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: leadId, status: newStatus, last_contacted_at: new Date().toISOString() }),
            });
            if (!res.ok) throw new Error('Failed');
    ...
    const getStatusBadge = (status: string | undefined) => {
        ...
        return (
            <span className={`px-2.5 py-1.5 rounded-md text-xs font-medium border border-transparent ${(status && badges[status]) || 'bg-[var(--foreground-muted)]/10 text-[var(--foreground-muted)]'}`}>
                            ...
                            <div className="bg-[#f6f8fa] dark:bg-[#161b22] rounded-lg border border-[var(--border)] p-4">
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex items-center justify-between pt-2 mt-2 border-t border-[var(--border)]">
                                        ...
                                        <span className="text-[var(--foreground)] font-medium">{selectedLead.created_at ? formatDistanceToNow(new Date(selectedLead.created_at), { addSuffix: true }) : '—'}</span>
                                    </div>
                                </div>
                            </div>
